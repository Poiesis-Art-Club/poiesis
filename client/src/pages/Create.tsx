...
export default function Create() {
  ...
  return <SiteShell><section className="studio-page"><header className="studio-header"><p className="folio-label">The studios</p><h1>Make in your medium.<br /><em>Meet another.</em></h1><p>Poiesis is a practical space for objects, images, sound, movement, craft, publishing, culture and digital work. Bring a tool, a draft, a material or a skill to share.</p></header>
    <section className="studio-wheel"><div className="studio-wheel__copy"><Sparkles size={20}/><p className="folio-label">A starting point</p><h2>Choose a practice,<br />a next step.</h2><p>Use this draw when you want a concrete way into the work.</p><button className="draw-prompt" onClick={draw}><Dices size={18}/>{prompt ? "Draw again" : "Draw a studio prompt"}</button></div><div className="studio-cards">{prompt ? <><article><p>Practice</p><strong>{prompt.practice}</strong></article><article><p>Starting point</p><strong>{prompt.start}</strong></article><article><p>Invitation</p><strong>{prompt.invitation}</strong></article></> : <><article><p>Practice</p><strong>Any medium</strong></article><article><p>Starting point</p><strong>One material</strong></article><article><p>Invitation</p><strong>One collaboration</strong></article></>}</div></section>
